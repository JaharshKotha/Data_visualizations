...
Bubbles = () ->
  ...
  chart = (selection) ->
    selection.each (rawData) ->
      ...
      data_full = transformData(rawData)
      ...
      data = (i for i in data_full when i.word is selected)
      console.log data
...
$ ->
  ...
  # [A] if there is nothing in the URL, consider the first item in the drop down
  if !text
    text = texts[0]
  ...
  # load our data
  # [A] call display function
  d3.csv("data/user_word_data.csv", display)